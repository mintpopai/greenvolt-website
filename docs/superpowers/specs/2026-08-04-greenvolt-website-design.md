...
- **媒体查询一律写 `max-width` / `min-width`，禁止用 Media Queries Level 4 的 range 语法（`@media (width < 900px)`）**。range 语法要 Chrome 104+ / Safari 16.4+ / Firefox 102+，在更老的浏览器上**整条规则会被直接丢弃**（不是降级），后果是移动端拿到桌面栅格。为覆盖旧 iOS Safari，这条是硬规则。
- **文字色阶的透明度是按 WCAG AA 反推出来的，不是凭手感调的**。以全站最亮的区块底色（媒体网络面板 `#0C2A22`）为最坏情况，实测：soft 9.0:1 / mute 6.5:1 / faint 4.7:1（正文门槛 4.5:1），`.gv-dim` 3.5:1（大字号门槛 3:1）。改这几个值前请重算，别直接往下压。
